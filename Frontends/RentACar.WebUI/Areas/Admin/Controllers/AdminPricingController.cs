...
namespace RentACar.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminPricingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7055/api/Pricings");
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricingDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7055/api/Pricings", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7055/api/Pricings?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7055/api/Pricings/{id}");
        ...
        public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricingDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7055/api/Pricings/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminPricing", new { area = "Admin" });
            }
